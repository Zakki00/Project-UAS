...
public class KaryawanController implements Initializable {
    ...
    @FXML
    private TableColumn<AbsensiModel, String> colAbsJabatan;
    @FXML
                ...
                return new TableCell<AbsensiModel, String>() {
                    ...
                    protected void updateItem(String item, boolean empty) {
                        ...
                        switch (item.toLowerCase()) {
                            case "hadir":
                                badge.getStyleClass().add("badge-hadir");
                                break;
                            case "izin":
                                badge.getStyleClass().add("badge-izin");
                                break;
                            case "sakit":
                                badge.getStyleClass().add("badge-sakit");
                                break;
                            default:
                                badge.getStyleClass().add("badge-alpha");
                                break;
                        }
                        setGraphic(badge);
                        setText(null);
    ...
    public void loadDataKaryawan() {
        ...
        try {
            List<Object[]> data = koneksi.ambilData(
                    "SELECT * FROM tb_karyawan ORDER BY id_karyawan");
        ...
        try {
            ...
            for (Object[] row : data) {
                ...
                absensiList.add(new AbsensiModel(
                        ...
                        row[3] != null ? row[3].toString() : "",
                        tanggal,
                        row[5] != null ? row[5].toString() : "Siang",
                        row[6] != null ? row[6].toString() : ""));
            }
    ...
    private boolean validasiKaryawan() {
        ...
        if (!txtNoHp.getText().matches("\\d{10,13}")) {
            showAlert(Alert.AlertType.WARNING,
                    "No HP harus terdiri dari 10 sampai 13 digit!");
            return false;
        }
    ...
    public void simpanKaryawan() {
        ...
        koneksi.eksekusiQuery(
                "UPDATE tb_karyawan " +
                        "SET nama_karyawan=?, jenis_kelamin=?, jabatan=?, no_hp=?, " +
                        "tanggal_masuk=?, status_kerja=?, alamat=? " +
                        "WHERE id_karyawan=?",
    ...
    public void hapusKaryawan() {
        ...
        Alert confirm = new Alert(
                ...
                "Hapus karyawan " + selected.getNamaKaryawan() + "?",
                ButtonType.YES,
                ButtonType.NO);
    ...
    public void simpanAbsensi() {
        if (dpTanggalAbsensi.getValue() == null) {
            showAlert(Alert.AlertType.WARNING, "Pilih tanggal absensi!");
            return;
        }
        if (cmbPilihKaryawan.getValue() == null) {
            showAlert(Alert.AlertType.WARNING, "Pilih karyawan!");
            return;
        }
        if (cmbShift.getValue() == null) {
            showAlert(Alert.AlertType.WARNING, "Pilih shift!");
            return;
        }
        if (cmbStatusKehadiran.getValue() == null) {
            showAlert(Alert.AlertType.WARNING, "Pilih status kehadiran!");
            return;
        }
        ...
        String jabatan = "-";
        for (KaryawanModel k : karyawanList) {
            if (k.getIdKaryawan().equals(idKary)) {
    ...
    public void ubahAbsensi() {
        AbsensiModel selected = tableAbsensi.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showAlert(Alert.AlertType.WARNING, "Pilih absensi di tabel terlebih dahulu!");
            return;
        }
        if (dpTanggalAbsensi.getValue() == null || cmbShift.getValue() == null
                || cmbStatusKehadiran.getValue() == null) {
            showAlert(Alert.AlertType.WARNING, "Lengkapi form absensi!");
            return;
        }
